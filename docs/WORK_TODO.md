...
| T-058 | P0 | B2B lead generation | done | Add secure MiniMax demand understanding proxy | Uses `MINIMAX_API_KEY` from local environment only. Frontend calls local `/api/ai/understand-demand`; server calls MiniMax and returns normalized product type, customer types, search terms, countries, cities, and reasoning. Missing key/API errors fall back to local rules. Never commit real keys. |
