...
| T-017 | P0 | Data source | todo | Define data source adapter contract | One normalized intake contract must support manual CSV/Excel upload now and optional API/scheduled sync later. |
...
| T-037 | P1 | Listings | todo | Build translation and localization workflow | Turn selected opportunities into localized listing content with translated title, attributes, keywords, compliance checks, and manual approval before publishing. |
...
- Localized listing content: translated title, attributes, keywords, risk notes, and publish-readiness status.
