...
| T-054 | P0 | B2B lead generation | todo | Add visible store-card extraction from active browser page | Future browser-assisted adapter: operator opens a generated source page and explicitly extracts visible store cards into the same preview-confirm lead pool flow. Must not store cookies, bypass login, bypass CAPTCHA, or collect hidden/private contacts. |
