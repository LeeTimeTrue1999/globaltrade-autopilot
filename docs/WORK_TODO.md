...
- B2B external source query runs that record source URLs, parser handoff, parsed lead counts, and confirmed lead counts.
- B2B one-click lead generation from product intent into demand research, source plans, query runs, and store leads.
- B2B no-API browser-visible page capture through a local bookmarklet handoff.
- B2B semi-automatic browser-assisted capture queue that opens source pages and returns visible results to the correct source plan.
- B2B MiniMax semantic understanding proxy for product intent, customer type, search term, and country/city recommendations.
- B2B low-frequency public contact lookup for one approved public page at a time, with rate limits, skip rules, and preview-confirm before lead-pool entry.
...
| T-052 | P0 | B2B lead generation | done | Add external source query and recording chain | 需求探查页现在可以打开生成的信息源，并点击“记录查询”写入外部查询批次；点击“带入解析”后自动填充平台、国家、城市、关键词和来源 URL；预览和确认线索会回写解析数量、入池数量、状态和审计日志。 |
...
| T-054 | P0 | B2B lead generation | done | Add no-API browser-visible page capture | 需求探查页新增“无 API 网页采集”：复制采集书签后，操作者可在 Google/Bing/地图/目录搜索页点击书签，将当前可见文本带回本地系统并自动进入线索预览。该方式不后台爬取、不保存 cookie、不绕登录/验证码。 |
| T-055 | P0 | B2B lead generation | done | Add one-click semantic lead generation UX | 需求探查页顶部已改为“输入想卖的商品 -> 一键生成店铺线索”。系统会自动完成语义分类、国家/城市推荐、来源计划、查询记录和待验证店铺线索入池；真实电话/网站/评分回填仍需后续 Google Places API 或浏览器可见页抽取适配。 |
| T-056 | P1 | B2B lead generation | todo | Add approved POI/API enrichment adapter | Add Google Places, local map APIs, or approved POI providers for stable real phone, website, rating, address, place id, and open-status enrichment with skip reasons and quota tracking. |
| T-057 | P0 | B2B lead generation | done | Add semi-automatic browser-assisted capture queue | 需求探查页新增“半自动浏览器采集”工作台。信息源可一键开始采集：系统选中来源、复制带 planId 的采集书签、打开外部搜索页；外部页点击书签回流后会归档到对应来源计划并进入线索预览。 |
| T-058 | P0 | B2B lead generation | done | Add secure MiniMax demand understanding proxy | Proxy implementation is in place and uses the official OpenAI-compatible endpoint `https://api.minimaxi.com/v1/chat/completions`. Local `.env.local` is loaded; `鱼竿` can return MiniMax-backed customer types, search terms, countries, cities, local keywords, reasoning, and confidence. Never commit real keys. |
| T-059 | P0 | B2B lead generation | done | Add low-frequency public contact lookup MVP | 需求探查页新增“自动低频查询当前来源”。本地服务端一次只读取一个公开文本页面，默认至少间隔 20 秒，不翻页、不递归、不保存 cookie，跳过 Google Maps/社交/登录或验证码风险高的平台；返回文本进入现有线索预览，确认后才入池。 |
| T-060 | P0 | UI | done | Refocus frontend on ToB lead workflow | Sidebar now hides old cross-border marketplace pages from the main navigation and keeps ToB lead overview, demand exploration, lead review, data sources, MVP status, and settings. Demand exploration is split into five workflow steps: input demand, judge markets, generate sources, collect/query, and confirm leads. |
...
- B2B lead source query runs: plan id, task id, source URL, recorded time, parsed time, confirmed time, parsed lead count, confirmed lead count, query status, and parser handoff fields.
- B2B one-click generated leads: product intent, inferred product type, generated source URL, candidate store name, target country/city, customer type, validation status, confidence level, and source mode.
- B2B browser-visible captures: source page URL, page title, captured visible text, capture time, linked source plan, parsed candidate count, skipped rows, and confirmation outcome.
- B2B semi-automatic browser capture sessions: source plan id, query run id, opened source URL, opened time, capture status, bookmarklet payload plan id, parsed count, confirmed count, and operator confirmation.
- B2B AI demand understanding: product intent, AI provider/model, inferred product type, target customer types, search terms, target countries/cities, demand signals, reasoning, confidence, and fallback mode.
- B2B low-frequency public contact lookup: source plan id, source URL, fetched time, lookup mode, min interval, skip reason, text length, parsed count, no-recursive-crawl flag, and no-cookie flag.
...
- Local AI keys stay in `.env.local` / environment variables and are ignored by git. `.env.example` may contain only empty placeholders.
...
| 2026-07-15 | Added automatic lead source discovery plans. Demand exploration now generates map, review, directory, and search-source URLs from product intent, country, city, and customer type, with parser mode, expected fields, safety rules, status tracking, local persistence, and audit logs. | `node --check app/src/app.js`; `node --check scripts/serve.js`; `npm.cmd test`; in-app browser check for demand generation, source plan generation, source URL/field/safety display, status update, reload persistence, and console errors. | Real active-page extraction from generated sources remains T-054; current implementation standardizes source discovery and parser routing. Existing untracked `docs/codex.code-workspace` left untouched. |
| 2026-07-15 | Added external source query and recording chain. Operators can open generated source URLs, record a query run, bring that run into the parsing form with source context prefilled, and have preview/confirmation update parsed and confirmed lead counts. | `node --check app/src/app.js`; `node --check scripts/serve.js`; `npm.cmd test`; in-app browser check for query recording without leaving the workbench, parser prefill, preview count write-back, confirmed count write-back, test lead cleanup, and console errors. | True active-page store-card extraction remains T-054; this local MVP records the end-to-end query handoff without storing cookies or bypassing external site controls. Existing untracked `docs/codex.code-workspace` left untouched. |
| 2026-07-15 | Added one-click semantic lead generation UX. The demand page now lets an operator enter a product intent and directly generate demand research, source plans, query runs, and待验证 store leads without manually opening or pasting sources. | `node --check app/src/app.js`; `node --check scripts/serve.js`; `npm.cmd test`; in-app browser check for one-click form, generated source plans, generated query records,待验证 lead rows, Google Places API notice, all navigation sections rendering, no horizontal overflow, and console errors. | Current one-click mode uses local semantic generation and source URLs; Google Places API or browser-assisted extraction remains needed for real phone/rating/address enrichment. Existing untracked `docs/codex.code-workspace` left untouched. |
| 2026-07-16 | Added no-API browser-visible page capture. The demand page now provides a capture bookmarklet that can run on an opened search/map/directory page, send visible text back via URL hash, and create a store-lead preview tied to the selected source plan. | `node --check app/src/app.js`; `node --check scripts/serve.js`; `npm.cmd test`; in-app browser check for capture panel, bookmarklet content, hash return import, lead preview creation, and test draft cleanup. | This is a browser-assisted visible-content handoff, not a background crawler. Stable real enrichment remains T-056. Existing untracked `docs/codex.code-workspace` left untouched. |
| 2026-07-17 | Added semi-automatic browser-assisted B2B capture queue. The demand page can now start capture from a source plan, copy a plan-bound capture bookmarklet, open the external source page, and return visible results to the matching source plan preview. | `node --check app/src/app.js`; `npm.cmd test`; in-app browser check for `鱼竿` demand entry, semi-auto capture queue visibility, plan-bound bookmarklet, visible-page return import, lead preview creation, and cleanup of test draft/test local records. | This keeps the MVP no-API and browser-assisted: no cookie storage, no hidden data, no CAPTCHA bypass. Existing untracked `docs/codex.code-workspace` left untouched. |
| 2026-07-17 | Added secure MiniMax demand-understanding proxy. Local dev server can read `MINIMAX_API_KEY` from `.env.local` or environment variables, call MiniMax server-side, normalize recommendations, and let the frontend fall back to local rules when no key or API failure occurs. | `node --check app/src/app.js`; `node --check scripts/serve.js`; `npm.cmd test`; local API check for `/api/ai/understand-demand` without a key returning `mode: local_fallback`; secret scan for exposed provider key prefixes and non-empty `MINIMAX_API_KEY`. | The user-exposed key in chat must be treated as compromised and was not used or committed. `.env.local` is git-ignored; only `.env.example` is committed. Existing untracked `docs/codex.code-workspace` left untouched. |
| 2026-07-17 | Tested MiniMax proxy configuration and hardened the prompt/parser sent to the provider. | `node --check scripts/serve.js`; `npm.cmd test`; local API checks for `/api/ai/understand-demand` with English `fishing rod` and UTF-8 Chinese `鱼竿`, both returning `mode: "minimax"`. | The provider key is loaded from `.env.local`; the prompt is now ASCII English to avoid existing file-encoding issues, and the parser strips MiniMax `<think>` blocks before extracting JSON. Real phone/website enrichment still needs semi-automatic visible-page capture or a future approved POI/API adapter. Existing untracked `docs/codex.code-workspace` left untouched. |
| 2026-07-17 | Added low-frequency public contact lookup MVP. Demand exploration can now trigger a server-side one-page public text lookup from the selected source plan and route the result into the existing preview-confirm lead parser. | `node --check app/src/app.js`; `node --check scripts/serve.js`; `npm.cmd test`. | This is intentionally not a crawler: one URL per request, default 20-second interval, no recursion, no cookies, and high-risk map/social/login sources are skipped to the browser-assisted flow. Existing untracked `docs/codex.code-workspace` left untouched. |
| 2026-07-17 | Refocused the frontend around the ToB lead workflow. The main navigation now hides old marketplace operations pages, the dashboard and report pages speak in lead-discovery terms, and demand exploration is organized as a five-step workspace instead of one crowded page. | `node --check app/src/app.js`; `node --check scripts/serve.js`; `npm.cmd test`; in-app browser check for ToB-only nav, five demand steps, step switching, no console errors, and no horizontal overflow. | Old cross-border marketplace pages remain in code for possible future reuse but are no longer exposed in the main navigation. Existing untracked `docs/codex.code-workspace` left untouched. |
