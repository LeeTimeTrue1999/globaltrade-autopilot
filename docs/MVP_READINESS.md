...
| B2B MiniMax semantic understanding | 需求探查 / local server | 已完成 | Frontend calls local `/api/ai/understand-demand`; the dev server reads `MINIMAX_API_KEY` from `.env.local` or environment variables and falls back to local rules when unavailable. |
...
4. One-click B2B leads can use MiniMax for semantic recommendations when a local key is configured, but real phone, website, rating, and address enrichment still needs Google Places API, approved POI APIs, or structured visible-page extraction.
...
9. Supplier discovery refresh is manual in the local MVP; background schedules need a hosted job runner later.
10. KPI weekly review needs clearer metrics and test outcome tracking.
11. Strategy recommendations need an approval/action log.
12. Shipment tracking is simulated; real carrier/platform sync is deferred.
13. Multi-user permissions and backend persistence are intentionally out of scope for the local MVP.
